...
THREE.GenericWireframeMaterial = function (parameters) {
	...
	console.log('COLOR: '+this._color.r+','+this._color.b+','+this._color.g);
}
...
THREE.GenericWireframeMaterial.prototype._fragmentShader = function () {
	...
	shader.push('float xPos = vPosition.x / '+parseFloat(this._repeat).toFixed(10)+';');
	shader.push('float zPos = vPosition.z / '+parseFloat(this._repeat).toFixed(10)+';');
	
	shader.push('float lowVal = 0.0 + '+parseFloat(this._width).toFixed(10)+';');
	shader.push('float highVal = 1.0 - '+parseFloat(this._width).toFixed(10)+';');

	shader.push('if( fract(xPos) < lowVal || fract(xPos) > highVal || fract(zPos) < lowVal || fract(zPos) > highVal ) { alpha = 1.0; }');

	console.log('RED: '+this._color.r);
